...
class TravelPreferenceList(generics.ListCreateAPIView):
    ...
    def get(self, request, *args, **kwargs):
        """
        Return the travel preference for the requested user
        or the logged-in user by default.
        """
        username = request.query_params.get("user", None)

        if username:
            preference = TravelPreference.objects.filter(
                owner__username=username).first()
        else:
            preference = TravelPreference.objects.filter(
        ...
        return Response({"detail": "No travel preference found."},
                        status=status.HTTP_404_NOT_FOUND)

    def perform_create(self, serializer):
        """
        Ensure that the user can only have one travel preference
        """
        if TravelPreference.objects.filter(owner=self.request.user).exists():
            raise serializers.ValidationError(
                    "You can only have one travel preference.")
...
class TravelPreferenceDetail(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_queryset(self):
        """
        Ensure that only the travel preference of the logged-in user
        is accessible.
        """
